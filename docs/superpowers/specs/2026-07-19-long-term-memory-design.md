...
不触发迁移。每次 v1/v2 读写都对 `sqlite_master` 执行精确对象 allowlist：只允许
当前版本预期的两张或四张表，以及 SQLite 为这些表的 PRIMARY KEY/UNIQUE 约束生成
的精确自动索引。任何额外 view、trigger、显式用户 index、表或带欺骗性 `sqlite`
前缀的对象都拒绝；合法自动索引不会被误拒绝。随后验证列名、声明类型、`NOT NULL`、
主键位置、无隐藏/generated 意外列、外键及 `ON DELETE CASCADE`、必需唯一/主键
索引，以及去除 SQL 注释后仍真实存在的角色、非空正文/标签和 64 字符 identity
关键 `CHECK`。任何其他版本或结构不匹配都使用安全的无效存储错误拒绝，不猜测或覆盖。
...
- v1/v2 的额外 view、trigger、显式 index、欺骗性 SQLite 前缀对象及隐藏/generated 列均被拒绝；
- 迁移失败回滚且不留下部分表或错误版本；
